...
```sh
moonx colmugx/ai-passport/cmd/passport@0.2.4 dev
```
...
```sh
moonx colmugx/ai-passport/cmd/passport@0.2.4 doctor --host folotoy-ai-passport
moonx colmugx/ai-passport/cmd/passport@0.2.4 build --host folotoy-ai-passport
```
...
rule(
  ...
  command: "moonx colmugx/ai-passport/cmd/passport@0.2.4 generate-sounds $input $output",
)
...
Forest Walk keeps its original **120×160 pixel-art design space** even though the SDK/Host framebuffer is **240×320**. The three parallax layers, fairy and battery HUD are all authored/rendered in that design coordinate system, and each design pixel is presented as one **2×2 nearest-neighbour block** on the final Canvas. The scenery layers therefore retain only 57.6 KB of index RAM (three 120×160 byte maps) on the no-PSRAM ESP32-C3, while the scene preserves its original sprite-to-world proportions instead of mixing low-resolution backgrounds with full-resolution foreground UI.
...
```sh
moonx colmugx/ai-passport/cmd/passport@0.2.4 doctor --host web
moonx colmugx/ai-passport/cmd/passport@0.2.4 build --host web
```
